...
NTSTATUS RegistryNotify(PVOID context, PVOID arg1, PVOID arg2) {
	UNREFERENCED_PARAMETER(context);
	UNREFERENCED_PARAMETER(arg2);
	if ((REG_NOTIFY_CLASS)(ULONG_PTR)arg1 != RegNtPostSetValueKey) return STATUS_SUCCESS;
	if (!g_Globals.isLoggingActive) return STATUS_SUCCESS;
	ULONG pid = HandleToULong(PsGetCurrentProcessId());
	if ((pid != g_Globals.targetPID)) return STATUS_SUCCESS;
	REG_POST_OPERATION_INFORMATION* info = (REG_POST_OPERATION_INFORMATION*)arg2;
	if (!NT_SUCCESS(info->Status)) return STATUS_SUCCESS;

	UNICODE_STRING fileName = RTL_CONSTANT_STRING(L"\\SystemRoot\\RegistryLoggerLog.txt");
	...
	if (vkInfo == NULL) return STATUS_SUCCESS;
